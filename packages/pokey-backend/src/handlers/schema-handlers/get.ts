...
export function createSchemaGetHandler(deps: HandlerDependencies): Handler {
  return async (request) => {
    const endTimer = deps.observability.startTimer(MetricEvent.SCHEMA_GET);
    try {
      deps.observability.trackEvent(MetricEvent.SCHEMA_GET);
      ...
      deps.observability.logError({ message: 'Failed to get schema', code: ErrorCode.INTERNAL_ERROR, details: error });
